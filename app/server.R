...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$do, {
    ...
    cat("this is after the dataset input so what's up?")
    ...
    # check if edgeR is in the list if so then run the edgeR
    if (length(input$differential_callers) > 4) {
      showModal(modalDialog(
        title = "Important!",
        "You selected more than 4 differential expression callers.  Please revise list.",
        easyClose = TRUE
      ))
      return()
    }
